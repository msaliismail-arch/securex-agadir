...
export default async function HomePage() {
  ...
  const stepsSubtitle = content["steps.subtitle"] ?? "Quatre étapes pour un contrôle technique sans tracas.";
  const featuresTitle = content["features.title"] ?? "Pourquoi nous choisir ?";
  const featuresSubtitle = content["features.subtitle"] ?? "Une expérience de contrôle technique moderne, fiable et sans surprise.";
  ...
  const contactSubtitle = content["contact.subtitle"] ?? "Facile d'accès au Quartier Industriel d'Agadir.";
  const ctaTitle = content["cta.title"] ?? "Prêt à passer le contrôle technique ?";
  const ctaSubtitle = content["cta.subtitle"] ?? "Réservez votre créneau en ligne dès maintenant et évitez l'attente. Certification officielle garantie.";
  ...
  return (
    <>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
            <Reveal>
              ...
              <h1 className="mt-6 text-4xl font-bold leading-[1.08] tracking-tight text-white md:text-5xl lg:text-6xl">
                <HighlightTitle title={heroTitle} highlight={heroHighlight} highlightClassName="text-hero-gradient" />
              </h1>
              <p className="mt-6 max-w-xl text-lg leading-relaxed text-white/85">{heroSubtitle}</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Button asChild size="lg" className="h-12 bg-white px-7 text-base text-primary shadow-glow hover:bg-white/90">
                  <Link href="/rendez-vous"><CalendarCheck className="mr-2 h-5 w-5" /> {ctaPrimary}</Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="h-12 border-white/40 bg-white/5 px-7 text-base text-white backdrop-blur-sm hover:bg-white/15">
                  <Link href="/tarifs">{ctaSecondary}<ChevronRight className="ml-1 h-4 w-4" /></Link>
                </Button>
              </div>
          ...
          {stats.map((s) => (
            <Reveal key={s.label}>
              <div className="rounded-2xl glass-card p-6 text-center shadow-soft">
                <div className="text-3xl font-bold text-primary md:text-4xl"><StatsCounter value={s.value} suffix={s.suffix} /></div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-sm font-semibold uppercase tracking-wider text-primary">Processus simple</p>
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">{stepsTitle}</h2>
          ...
          {STEPS.map((step, i) => (
            <Reveal key={step.title} delay={i * 0.08}>
              <div className="relative text-center">
                ...
                <span className="mt-4 inline-block text-xs font-bold uppercase tracking-wider text-primary">Étape {i + 1}</span>
          ...
          <Reveal>
            <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
              <div className="max-w-2xl">
                ...
                <h2 className="mt-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">Catégories de véhicules &amp; tarifs</h2>
              ...
              return (
                <Reveal key={cat.id} delay={i * 0.06}>
                  <Card className="group relative h-full overflow-hidden border-border p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card">
                    ...
                    <div className={`flex h-14 w-14 items-center justify-center rounded-xl ${c.soft} ${c.fg}`}><Icon className="h-7 w-7" /></div>
                    <h3 className="mt-5 text-lg font-semibold text-foreground">{cat.name}</h3>
                    <p className="mt-1.5 min-h-[40px] text-sm leading-relaxed text-muted-foreground">{cat.description}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-sm font-semibold uppercase tracking-wider text-primary">Notre engagement</p>
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">{featuresTitle}</h2>
          ...
          <div className="mx-auto max-w-7xl px-6">
            <Reveal>
              <div className="mx-auto max-w-2xl text-center">
                ...
                <h2 className="mt-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">Annonces &amp; promotions</h2>
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {announcements.slice(0, 3).map((a, i) => (
                <Reveal key={a.id} delay={i * 0.06}>
                  <Card className={`relative h-full border-border p-6 shadow-soft ${a.pinned ? "ring-1 ring-orange-300" : ""}`}>
                    {a.pinned && <Badge className="absolute -top-2.5 right-4 bg-orange-500 text-white">Épinglé</Badge>}
                    ...
                    <p className="mt-4 text-xs text-muted-foreground">
                      {new Date(a.publishedAt).toLocaleDateString("fr-FR", { day: "2-digit", month: "long", year: "numeric" })}
                    </p>
                  </Card>
                </Reveal>
              ))}
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">{testimonialsTitle}</h2>
          </div>
        </Reveal>
      ...
      <section className="bg-secondary/40 py-20">
        <div className="mx-auto max-w-7xl px-6">
          <Reveal>
            <div className="mx-auto max-w-2xl text-center">
              <p className="text-sm font-semibold uppercase tracking-wider text-primary">Contact &amp; localisation</p>
              <h2 className="mt-2 text-3xl font-bold tracking-tight text-foreground md:text-4xl">{contactTitle}</h2>
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-2">
            <Reveal>
              <div className="overflow-hidden rounded-2xl border border-border shadow-card">
                <iframe title="Localisation SÉCUREX CONNECT Agadir" src={MAPS_EMBED} width="100%" height="100%" loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="min-h-[360px] w-full" />
              </div>
            </Reveal>
            <Reveal delay={0.1}>
              <Card className="h-full border-border p-8 shadow-soft">
                <div className="space-y-6">
                  <ContactRow icon={MapPin} title="Adresse" accent><p className="text-sm text-muted-foreground">{BRAND.address}</p></ContactRow>
                  <ContactRow icon={Phone} title="Téléphone" accent><a href={`tel:${BRAND.phoneRaw}`} className="text-sm text-muted-foreground transition-colors hover:text-primary">{BRAND.phone}</a></ContactRow>
                  <ContactRow icon={Mail} title="Email" accent><a href={`mailto:${BRAND.email}`} className="text-sm text-muted-foreground transition-colors hover:text-primary">{BRAND.email}</a></ContactRow>
                  <ContactRow icon={Clock} title="Horaires" accent><p className="text-sm text-muted-foreground">{BRAND.hours}</p></ContactRow>
      ...
      <section className="bg-brand-gradient py-20">
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <h2 className="mt-5 text-3xl font-bold tracking-tight text-white md:text-4xl">{ctaTitle}</h2>
          <p className="mx-auto mt-4 max-w-xl text-white/85">{ctaSubtitle}</p>
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Button asChild size="lg" className="h-12 bg-white px-8 text-base text-primary shadow-float hover:bg-white/90">
              <Link href="/rendez-vous"><CalendarCheck className="mr-2 h-5 w-5" /> {ctaPrimary}</Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 border-white/40 bg-white/10 px-8 text-base text-white hover:bg-white/20 hover:text-white">
...
function ContactRow({ icon: Icon, title, children, accent }: { icon: LucideIcon; title: string; children: any; accent?: boolean }) {
  return (
    <div className="flex gap-4">
      <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${accent ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
...
function SocialLink({ href, label }: { href: string; label: string }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-background text-muted-foreground shadow-soft transition-all hover:bg-primary hover:text-white hover:shadow-glow">
