...
export default function CheckinPage() {
  ...
  return (
    <div className="w-full space-y-5">
      {/* Counter */}
      <div className="flex items-center justify-between rounded-2xl bg-orange-50 border border-orange-200 px-5 py-3">
        <div className="flex items-center gap-2.5">
          <div className="rounded-full bg-[#F2994A] p-1.5">
          ...
          <div>
            ...
            <div className="text-2xl font-bold text-[#F2994A] leading-tight">
      ...
      <Tabs value={mode} onValueChange={(v) => setMode(v as Mode)} className="w-full">
        <TabsList className="grid w-full grid-cols-2 h-12">
          <TabsTrigger value="scan" className="text-sm gap-1.5 data-[state=active]:bg-[#F2994A] data-[state=active]:text-white">
            <ScanLine className="h-4 w-4" />
            Scan QR
          </TabsTrigger>
          <TabsTrigger value="manual" className="text-sm gap-1.5 data-[state=active]:bg-[#F2994A] data-[state=active]:text-white">
...
function ScanMode({
  ...
  return (
    <div className="space-y-4">
      <div className="relative rounded-2xl overflow-hidden bg-navy aspect-square max-w-md mx-auto ring-4 ring-orange-200/50">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="relative w-[250px] h-[250px]">
            {/* corners */}
            <span className="absolute top-0 left-0 h-8 w-8 border-t-4 border-l-4 border-[#F2994A] rounded-tl-lg" />
            <span className="absolute top-0 right-0 h-8 w-8 border-t-4 border-r-4 border-[#F2994A] rounded-tr-lg" />
            <span className="absolute bottom-0 left-0 h-8 w-8 border-b-4 border-l-4 border-[#F2994A] rounded-bl-lg" />
            <span className="absolute bottom-0 right-0 h-8 w-8 border-b-4 border-r-4 border-[#F2994A] rounded-br-lg" />
            {/* animated scan line */}
            <div className="absolute inset-x-2 top-0 h-0.5 bg-gradient-to-r from-transparent via-[#F2994A] to-transparent shadow-[0_0_8px_#F2994A] scan-line-anim" />
          </div>
        </div>
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
          <span
            className={cn(
              ...
              cameraActive ? "bg-emerald-500/90 text-white" : "bg-white/90 text-navy",
            )}
          >
          ...
          {verifying && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F2994A] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-white">
        ...
        {scanError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white gap-3 p-6 bg-navy/95">
            <CameraOff className="h-10 w-10 text-[#F2994A]" />
            <div className="text-sm font-semibold">Caméra indisponible</div>
            <p className="text-xs text-white/70 max-w-xs">{scanError}</p>
            <Button
              size="sm"
              onClick={onSwitchToManual}
              className="bg-[#F2994A] hover:bg-[#F2994A]/90 text-white mt-1"
            >
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      <div className="rounded-2xl bg-orange-50 border border-orange-200 p-5">
        <div className="flex items-center gap-2 mb-3">
          <Hash className="h-4 w-4 text-[#F2994A]" />
          <span className="text-sm font-semibold text-navy">Code à 6 caractères</span>
        </div>
        <div className="flex items-center justify-center gap-2 sm:gap-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <input
              ...
              maxLength={1}
              className="h-14 w-12 sm:h-16 sm:w-14 rounded-xl border-2 border-orange-200 bg-white text-center font-mono text-2xl sm:text-3xl font-bold text-navy uppercase focus:border-[#F2994A] focus:ring-2 focus:ring-[#F2994A]/30 outline-none transition-all"
            />
          ))}
        </div>
      ...
      <Button
        ...
        className="w-full h-14 text-base font-semibold bg-[#F2994A] hover:bg-[#F2994A]/90 text-white shadow-sm disabled:opacity-50"
      >
        {verifying ? (
          <>
...
function SuccessCard({
  ...
  return (
    <div className="rounded-3xl overflow-hidden ring-4 ring-emerald-200 shadow-xl">
      {/* Banner */}
      <div className="bg-emerald-600 px-6 py-8 text-center text-white">
        ...
        <p className="text-emerald-50 text-sm mt-1">Le rendez-vous est valide · Bienvenue</p>
      </div>

      {/* Queue number — huge */}
      <div className="bg-white px-6 py-6 text-center border-b border-border">
        <div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground font-semibold">
          Numéro de file
        </div>
        <div className="text-7xl sm:text-8xl font-bold text-[#F2994A] leading-none mt-1 tracking-tight">
          {appt.queueNumber ?? "—"}
        </div>
        <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 border border-emerald-200">
      ...
      {/* Counter footer */}
      <div className="bg-orange-50 px-5 py-3 flex items-center justify-between border-t border-orange-100">
        <span className="text-xs text-orange-700 font-medium">Validations aujourd&apos;hui</span>
        <span className="text-lg font-bold text-[#F2994A]">{todayCount ?? "—"}</span>
      </div>

      <div className="bg-white p-4">
        <Button onClick={onReset} size="lg" className="w-full h-14 text-base bg-[#F2994A] hover:bg-[#F2994A]/90 text-white">
...
function WarningCard({ appt, onReset }: { appt: Appointment; onReset: () => void }) {
  ...
  return (
    <div className="rounded-3xl overflow-hidden ring-4 ring-amber-200 shadow-xl">
      <div className="bg-amber-500 px-6 py-8 text-center text-white">
        ...
        <p className="text-amber-50 text-sm mt-1 max-w-sm mx-auto">{meta.desc}</p>
      </div>
      ...
      <div className="bg-white p-4">
        <Button onClick={onReset} size="lg" variant="outline" className="w-full h-14 text-base border-[#F2994A] text-[#F2994A] hover:bg-orange-50">
          <RotateCcw className="h-5 w-5 mr-2" />
          Réessayer
        </Button>
      </div>
...
function NotFoundCard({ message, onReset }: { message: string; onReset: () => void }) {
  return (
    <div className="rounded-3xl overflow-hidden ring-4 ring-red-200 shadow-xl">
      <div className="bg-red-600 px-6 py-10 text-center text-white">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ delay: 0.1, type: "spring", stiffness: 260, damping: 16 }}
        ...
        <p className="text-red-50 text-sm mt-1 max-w-sm mx-auto">{message}</p>
      </div>

      <div className="bg-white p-5">
        <div className="rounded-lg bg-surface p-3 text-xs text-muted-foreground space-y-1">
          <div className="font-semibold text-navy">Vérifiez :</div>
...
function DetailRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg bg-surface px-3 py-2.5">
      <div className="rounded-md bg-white p-1.5 ring-1 ring-border">
        <Icon className="h-4 w-4 text-[#F2994A]" />
      </div>
      <div className="min-w-0 flex-1">
        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{label}</div>
        <div className={cn("text-sm font-semibold text-navy truncate", mono && "font-mono tracking-wider")}>
          {value}
        </div>
      </div>
    </div>
