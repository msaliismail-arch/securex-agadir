...
const BOOT_LINES = [
  "> SÉCUREX CONNECT — SYSTÈME DE CONTRÔLE v2.0",
...
export default function SelectAccountPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-black font-mono text-white">
      ...
      <div className="pointer-events-none absolute left-0 right-0 top-0 h-px bg-[#00FF88]/40 scan-line-anim" />

      {/* Top-left back link */}
      <a
        href="/"
        className="absolute left-4 top-4 z-20 inline-flex items-center gap-1.5 text-[11px] uppercase tracking-[0.18em] text-emerald-400/70 transition hover:text-emerald-300"
      >
      ...
      {/* Top-right status */}
      <div className="absolute right-4 top-4 z-20 hidden items-center gap-2 text-[11px] uppercase tracking-[0.18em] text-emerald-400/60 sm:flex">
      ...
      <main className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col px-5 pb-12 pt-16 sm:px-8 sm:pt-20">
        ...
        <section className="mb-8 sm:mb-12" aria-label="Séquence de démarrage">
          <pre className="whitespace-pre-wrap text-[12px] leading-relaxed text-emerald-300/90 sm:text-[13px]">
            ...
            {bootDone && (
              <div className="mt-1 text-emerald-500/70">
                &gt; <span className="text-emerald-400/80">accès protégé · 2FA obligatoire</span>
                <span className="cursor-blink ml-0.5 inline-block">▊</span>
              </div>
            )}
          </pre>
          ...
          {roles.map((role) => {
            ...
            return (
              ...
              >
                ...
                {/* Permissions */}
                <ul className="scroll-thin -mr-1 max-h-36 space-y-1 overflow-y-auto pr-1 text-[12px] leading-relaxed text-white/65">
        ...
        {/* Footer line */}
        <footer className="mt-10 flex items-center justify-between gap-3 text-[11px] uppercase tracking-[0.16em] text-emerald-500/40">
